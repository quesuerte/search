...
pub async fn backend_search(mut db: Connection<PostgresBackend>, input: &str, semantic: Option<&OllamaConfig>) -> Result<Vec<PgRow>, sqlx::Error> {
    ...
    let query = match semantic.is_some() {
        true => "SELECT a.id, b.uri, b.redirect, b.content_type, b.source, a.page, b.uri, b.title, b.author, b.summary, a.embedding <=> $1 AS rank \
                FROM semantic_search a \
                INNER JOIN sources b ON a.id = b.id \
                ORDER BY rank \
                LIMIT $2",
        false => "SELECT a.id, b.uri, b.redirect, b.content_type, b.source, a.page, b.uri, b.title, b.author, b.summary, ts_rank(a.ts, websearch_to_tsquery('english', $1)) AS rank \
